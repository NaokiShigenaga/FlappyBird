...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupWall() {
        ...
        let rendom_y_range = birdSize.height * 5
        
        //下の壁のY軸下限位置(中央位置から下方向の最大振れ幅で下の壁を表示する位置)を計算
        let groundSize = SKTexture(imageNamed: "ground").size()
        let center_y = groundSize.height + (self.frame.size.height - groundSize.height) / 2
        let under_wall_lowest_y = center_y - slit_length / 2 - wallTexture.size().height / 2 - rendom_y_range / 2
    ...
    func setupItem() {
        ...
        let groundSize = SKTexture(imageNamed: "ground").size()
        let center_y = groundSize.height + (self.frame.size.height - groundSize.height) / 2
        let item_lowest_y = center_y - slit_length2 / 2 - itemTexture.size().height / 2 - random_y_range2 / 2
        ...
        let createItemAnimation = SKAction.run({
            ...
            // 0〜random_y_rangeまでのランダム値を生成
            let random_y = CGFloat.random(in: 0..<random_y_range2)
            // Y軸の下限にランダムな値を足して、下の壁のY座標を決定
